  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        # Trusts SOP's own service-account identity (2026-09-01, "Wire
        # SOP's issue-side linkage to IM") - a third, deliberately-
        # provisioned Cognito app client (sop_im_service_account.tf),
        # additive to (never a replacement for) the human-facing and
        # POP-facing audiences above.
        { name = "IM_JWT_SERVICE_AUDIENCE_SOP", value = aws_cognito_user_pool_client.sop_im_service.id }
      ]
