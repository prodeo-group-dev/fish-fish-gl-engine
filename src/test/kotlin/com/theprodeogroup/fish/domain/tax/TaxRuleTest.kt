...
class TaxRuleTest {
    ...
    fun `given a valid jurisdiction, tax type, and rate, when created, then it holds the values`() {
        val rule = TaxRule("Sierra Leone", TaxType.CORPORATE_INCOME_TAX, BigDecimal("0.30"))
    ...
    fun `given a blank jurisdiction, when created, then it fails`() {
        shouldThrow<IllegalArgumentException> {
            TaxRule("", TaxType.CORPORATE_INCOME_TAX, BigDecimal("0.30"))
        }
    }
    ...
    fun `given a negative rate, when created, then it fails`() {
        shouldThrow<IllegalArgumentException> {
            TaxRule("Sierra Leone", TaxType.CORPORATE_INCOME_TAX, BigDecimal("-0.01"))
        }
    }

    @Test
    fun `given a zero rate, when created, then it succeeds`() {
        val rule = TaxRule("Sierra Leone", TaxType.CORPORATE_INCOME_TAX, BigDecimal.ZERO)
