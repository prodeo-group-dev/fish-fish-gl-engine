...
class PurchasePostingContextRoutesTest {
    ...
    private class Fixture(configureAccounts: Boolean = true, openPeriod: Boolean = true, configureFacilityLiability: Boolean = true) {
        ...
        val expenseAccount = if (configureAccounts) {
            Account.create(company.id, AccountType.EXPENSE, null, "5000", "Operating Expenses").also { accountRepository.save(it) }
        } else null
    ...
    @Test
    fun `given an open Period and a configured Chart of Accounts, when GET purchase-posting-context is called, then it returns the resolved ids`() = testApplication {
        val fixture = Fixture()
    ...
    @Test
    fun `given no bearer token, when GET purchase-posting-context is called, then it returns 401`() = testApplication {
        val fixture = Fixture()
        application { fixture.installInto(this) }
        val client = createClient { install(ContentNegotiation) { json() } }
    ...
    @Test
    fun `given a nonexistent Company, when GET purchase-posting-context is called, then it returns 404`() = testApplication {
        val fixture = Fixture()
        application { fixture.installInto(this) }
        val client = createClient { install(ContentNegotiation) { json() } }

        val response = client.get("/api/companies/${UUID.randomUUID()}/purchase-posting-context") {
            header(HttpHeaders.Authorization, "Bearer ${TestJwtSupport.signToken(ADMIN_EMAIL)}")
            header("X-Tenant-Id", fixture.tenant.id.value.toString())
        }
    ...
    @Test
    fun `given no open Period, when GET purchase-posting-context is called, then it returns 409`() = testApplication {
    ...
    @Test
    fun `given no Chart of Accounts, when GET purchase-posting-context is called, then it returns 409`() = testApplication {
