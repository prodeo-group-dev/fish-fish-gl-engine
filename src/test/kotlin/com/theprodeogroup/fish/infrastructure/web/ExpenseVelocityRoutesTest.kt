...
/** `GET /companies/{companyId}/expense-velocity` - see ExpenseVelocityRoutes.kt's own KDoc. */
class ExpenseVelocityRoutesTest {
    ...
    private class Fixture {
        ...
        val cogsAccount = Account.create(company.id, AccountType.EXPENSE, null, "5100", "Cost of Goods Sold", expenseClassification = ExpenseClassification.COST_OF_GOODS_SOLD).also { accountRepository.save(it) }

        fun postExpense(account: Account, amount: String) {
    ...
    @Test
    fun `given a matching X-Tenant-Id and bearer token, when GET expense-velocity is called, then it returns the daily rate excluding COGS`() = testApplication {
        val fixture = Fixture()
        fixture.postExpense(fixture.adminExpenseAccount, "500.00")
        fixture.postExpense(fixture.cogsAccount, "3000.00")
        application { fixture.installInto(this) }
        val client = createClient { install(ContentNegotiation) { json() } }

        val response = client.get("/api/companies/${fixture.company.id.value}/expense-velocity") {
        ...
        val body: ExpenseVelocityResponseDto = response.body()
    ...
    @Test
    fun `given no bearer token, when GET expense-velocity is called, then it returns 401`() = testApplication {
        val fixture = Fixture()
        application { fixture.installInto(this) }
        val client = createClient { install(ContentNegotiation) { json() } }
    ...
    @Test
    fun `given a caller authenticated in a different Tenant, when GET expense-velocity is called, then it returns 403`() = testApplication {
        val fixture = Fixture()
    ...
    @Test
    fun `given a nonexistent Company, when GET expense-velocity is called, then it returns 404`() = testApplication {
        val fixture = Fixture()
        application { fixture.installInto(this) }
        val client = createClient { install(ContentNegotiation) { json() } }

        val response = client.get("/api/companies/${java.util.UUID.randomUUID()}/expense-velocity") {
            header(HttpHeaders.Authorization, "Bearer ${TestJwtSupport.signToken(ADMIN_EMAIL)}")
            header("X-Tenant-Id", fixture.tenant.value.toString())
        }
